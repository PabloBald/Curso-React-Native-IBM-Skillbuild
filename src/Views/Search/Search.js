...
function normalizeString(str) {
  return str
    ...
    .replace(/([^n\u0300-\u036f]|n(?!\u0303(?![\u0300-\u036f])))[\u0300-\u036f]+/gi,'$1',); //elimina que no corresponde;
}
...
const Search = () => {
  ...
  useEffect(() => {
    ...
    const aa = async () => {
      const prov = await (await fetch('https://apis.datos.gob.ar/georef/api/provincias')).json();
      const loc = await (await fetch('https://apis.datos.gob.ar/georef/api/localidades?max=4142')).json();
...

        {
          isSearching
            ? (
              <FlatList
                data={filteredCities}
                renderItem={ (item) => <Button onPress={() => handlePressPredictivo(item.item)}>{item.item.nombre}</Button> }
